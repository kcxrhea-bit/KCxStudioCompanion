...
- **Ollama-enabled Spec Intake blocked by its own grounding (verified live 2026-07-21)**: `CortexExecutionPermissions.getSummarizeSafetyDiagnostic` scans `purpose + prompt` for `\bwrite\b|\bdelete\b|\bshell\b|\bbackground\b|…`. The Ollama-path prompt embeds the real `src/` tree, so benign file names trip the scan — this repo's `src/styles/background.css` / `DashboardBackground.tsx` guarantee a "blocked background token" match. Approval fails ("Only summarize-safe local requests can be approved") and `createSpecIntakeRequest` **returns without the KCxModeAI-brain fallback** (that fallback only wraps the execution step, lines ~727–733 vs ~755+). Net effect: with Ollama enabled, Spec Intake for this repo produces nothing; the only trace is in the chamber's Cortex Event Timeline. The same token scan applies to the build-loop summarize prompt, which embeds build-log tails (inferred, not live-verified: logs containing "write"/"background"/etc. would likewise be blocked).
...
- **Ollama-enabled state (verified live)**: chamber readiness becomes "active / Build loop operational | Inference ready"; ML runtime Loaded, model phi3:latest; Activation/Intelligence Active; AI Providers shows Ollama "configured" with a working Test Connection ("Local-only provider ready"). Cosmetic inconsistency: the chamber status grid's Ollama chip still reads "Dormant" in this state. Functional defect: see §6 — the Ollama Spec Intake path is blocked by the permission token scan for this repo.
