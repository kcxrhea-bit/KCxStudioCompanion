  ...
  return (
    <section className="guide-page">
      ...
      <G title="Workflows">
        ...
        <p><strong>Spec Intake (Cortex chamber):</strong> plain-English spec → SmartBrain normalizer (sanitizes risky phrasing, classifies task type + risk, records a trace) → if Ollama is enabled: grounded prompt including the real src/ tree (depth ≤ 4) runs through the manual execution queue + sandbox; otherwise the embedded KCxModeAI brain answers; final fallback is the rule-based structured prompt. Result lands in the Approval Queue. <strong>A project must be selected</strong> — without one the result is silently discarded (known gap). <strong>Ollama-path caveat (verified live):</strong> the execution permission layer scans the whole grounded prompt for blocked tokens (write, delete, shell, background, …). Because the embedded src/ tree can contain benign file names that match (this repo's <code>background.css</code> / <code>DashboardBackground.tsx</code> always do), approval can fail with "Only summarize-safe local requests can be approved" — and the pipeline then stops without falling back to the embedded brain, so nothing reaches the queue. The failure is only visible in the chamber's Cortex Event Timeline.</p>
      ...
      <G title="Troubleshooting">
        ...
        <p><strong>Spec Intake produces nothing even with Ollama enabled:</strong> open the Cortex chamber's Event Timeline. If you see "Spec intake approval failed — Only summarize-safe local requests can be approved", a blocked token (write / delete / shell / background / …) appeared in the generated prompt — often from a file name in the embedded src tree. There is currently no fallback in this case.</p>
