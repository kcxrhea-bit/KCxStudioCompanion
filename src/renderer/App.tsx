...
export function App() {
  ...
  const copyText = async (value: string, approvalId: string, message?: string): Promise<boolean> => {
    try {
      if (navigator.clipboard?.writeText) {
        await navigator.clipboard.writeText(value);
        setCopyStatus({ id: approvalId, message: message ?? "Copied" });
        return true;
      }
    } catch {
      // fallback below
  ...
  return <><div className="app-background"><div className="light-ray"></div><div className="light-ray"></div><div className="light-ray"></div><div className="light-ray"></div></div><div className={`shell app-shell section-${activeSectionKey}`}><aside className="sidebar panel-primary edge-glow-left"><div className="nav-group"><button type="button" onClick={() => setShowValhalla(true)}>Enter Valhalla</button></div><div className="nav-separator" /><div className="nav-group">{coreWorkflowNavItems.map((n) => renderNavItem(n))}</div><div className="nav-separator" /><div className="nav-group">{aiOperationsNavItems.map((n) => renderNavItem(n))}</div><div className="nav-separator" /><div className="nav-group">{developmentNavItems.map((n) => renderNavItem(n))}</div><div className="nav-separator" /><div className="nav-group">{systemNavItems.map((n) => renderNavItem(n))}</div><div className="nav-separator" /><div className="nav-group">{productInfoNavItems.map((n) => renderNavItem(n))}</div>{isDevelopment && <><div className="nav-separator" /><div className="nav-group">{devDebugNavItems.map((n) => renderNavItem(n))}</div></>}</aside><main>
    ...
    {tab === "Release Notes" && <section className="release-notes"><h3 className="title">Release Notes</h3>
      <article className="release-notes-entry">
        <h3>v0.9.5-beta</h3>
        <h4>What&apos;s New</h4>
        <p><strong>Spec Intake.</strong> Describe a feature in plain English in the Cortex chamber. The system reads your actual project file structure, normalizes the request through the Smart Brain pipeline, generates a structured Claude Code implementation prompt, and drops it directly into the Approval Queue. No copy-paste from a separate tool required.</p>
        <p><strong>Smart Brain Normalizer.</strong> Every prompt now passes through a classification and sanitization layer before it moves anywhere. It detects risky or vague requests, trims scope where needed, and records a normalization trace visible in the Approval Queue card so you can see exactly what changed.</p>
        <p><strong>Embedded KCxModeAI brain.</strong> A local fallback intelligence module is always running inside Studio Companion. When Ollama is offline, Spec Intake routes through this brain automatically — no setup, no external process. It shows up in AI Providers as &quot;Local KCx Brain&quot; and in the Valhalla runtime as GodzillaMode AI.</p>
        <p><strong>Ollama src tree injection.</strong> When Ollama is running, the generated prompt now includes your actual <code>src/</code> file tree (depth ≤ 4) instead of a flat file list. This prevents invented file names and keeps implementation steps grounded in what actually exists in the repo.</p>
        <h4>Improvements</h4>
        <p><strong>SERA policy system now active.</strong> 53 forbidden token patterns and 13 high-risk rules are enforced before any request reaches the execution pipeline. Shell chaining, remote execution payloads, and high-risk command patterns are blocked at the intake boundary. Audit log is available in the runtime.</p>
        <p><strong>ForgeCanvas node states are now live.</strong> System nodes on the Valhalla canvas derive their activity state from the current Cortex snapshot instead of hardcoded values. A connected bridge shows as active; a dormant external project shows as dormant.</p>
        <p><strong>Bridge registry honesty.</strong> Readiness percentages in the Bridge Readiness Matrix now reflect actual runtime context — embedded brain availability, telemetry data presence, build context, and memory entries. External app bridges are clearly labeled dormant at 0%.</p>
        <p><strong>Forge Bay dead-ends fixed.</strong> External system chambers (Android apps, KCx site, etc.) now show a disabled &quot;Bridge Not Configured&quot; button and explain that the project is registered in the ecosystem map but not connected. Previously the button was enabled and did nothing useful.</p>
        <p><strong>Prompt delivery confirmation.</strong> The Approval Queue now shows explicit confirmation text when you copy or mark a prompt sent: &quot;Prompt copied — manual handoff to Claude Code&quot; or &quot;Prompt marked sent — manual handoff to Claude Code&quot;. No more silent status changes.</p>
        <p><strong>Ollama offline UX.</strong> The Spec Intake panel now shows numbered setup steps with a copy button for the model pull command when Ollama is not running. No automatic installation — the steps are display-only and manual.</p>
        <h4>Removed</h4>
        <p>The Prompt Generator tab has been removed. It was a dead button that produced no output. The feature will return when the underlying generation pipeline is wired to it properly.</p>
        <h4>Stability</h4>
        <p>281 tests passing across 14 suites, up from 263 in v0.9.0-beta.1. All TypeScript errors resolved. Clean production build confirmed.</p>
        <h4>Known Limitations</h4>
        <p>Prompt-type approvals require manual copy to Claude Code. There is no automated delivery — the app cannot send prompts to Claude on your behalf.</p>
        <p>ForgeCanvas system nodes are positioned statically. The layout and edges do not reflect live topology or real connection paths between systems.</p>
        <p>External app bridges (Android apps, KCx site, Messenger Desktop, etc.) are ecosystem map placeholders. They will not connect or communicate from this app in this release.</p>
        <p>Ollama must be started manually before use. Studio Companion does not check Ollama liveness on launch or restart it automatically.</p>
      </article>
      <article className="release-notes-entry">
        <h3>v0.9.0-beta.1</h3>
        <p>Production packaging foundation, defensive storage recovery, diagnostics logging, product settings, release documentation, and paid-product architecture hooks.</p>
      </article>
    </section>}
